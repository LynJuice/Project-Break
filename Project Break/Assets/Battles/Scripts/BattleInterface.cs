...
public class BattleInterface : MonoBehaviour
{
    ...
    IEnumerator UseSkill(int Z)
    {
        ...
        { // heal
            ...
            {
                int HealAmount = Random.Range(hero.Spirit.Powers[Z].BaseHeal, hero.Spirit.Powers[Z].MaxHeal);
                BaseHero[] Heros = new BaseHero[BSM.HerosInBattle.Count];

                for (int i = 0; i < BSM.HerosInBattle.Count; i++)
                {
                    Heros[i] = BSM.HerosInBattle[i].GetComponent<HeroStateMachine>().hero;
                }

                for (int i = 0; i < Heros.Length; i++)
                {
                    Heros[i].CurHp += HealAmount;

                    if (Heros[i].CurHp > Heros[i].BaseHp)
                        Heros[i].CurHp = Heros[i].BaseHp;
                }
            }
